...
sg.set_global_icon('heatmap-icon.png')

# check if docker daemon is running
docker_error = subprocess.run(['/usr/local/bin/docker ps'], shell=True, capture_output=True, text=True).stderr
if docker_error:
...
layout = [
    [sg.Text('Project folder path:', background_color='#181818', pad=(5,6.5)), sg.InputText(key='projPath', default_text='/Users/petermenzies/Projects/heatmaps/app/heatmap-app-test', tooltip=' Where you want the config.json file to be saved '), sg.FolderBrowse(target='projPath', button_color='#37373C')],
    [sg.Text('Input path:', background_color='#181818', pad=(5,6.5)), sg.InputText(key='inputPath', default_text='/Users/petermenzies/Projects/heatmaps/app/heatmap-app-test/data-in', tooltip=' where your shapes are saved '), sg.FolderBrowse(target='inputPath', button_color='#37373C')],
    [sg.Text('Ouput path:', background_color='#181818', pad=(5,6.5)), sg.InputText(key='outputPath', default_text='/Users/petermenzies/Projects/heatmaps/app/heatmap-app-test/data-out', tooltip=' where you want output rasters to go '), sg.FolderBrowse(target='outputPath', button_color='#37373C')],
...
# Create the window
window = sg.Window('Heatmapp', layout, size=(475, 290), background_color='#181818')
